...
//  Copyright (c) 2016年 TIXA. All rights reserved.
//
...
-(void)createImageTopView
{
    ...
    [self.sectionImages enumerateObjectsUsingBlock:^(id   obj, NSUInteger idx, BOOL *  stop) {
    ...
    [self.sectionSelectedImages enumerateObjectsUsingBlock:^(id  obj, NSUInteger idx, BOOL * stop) {
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        //滚动到选中的视图
        [_rootScrollView setContentOffset:CGPointMake((_userSelectedChannelID )*self.bounds.size.width, 0) animated:YES];
    }
}
